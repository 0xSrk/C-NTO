      ...
      <div className={s.rows}>
        ...
        <p className={s.note}>
          {tr(
            ...
            'PnL is recomputed from prices and the CME registry point value, then reduced by commissions; the Profit column is a check. An instrument missing from the registry is skipped and reported (unrecognized instrument). Exact duplicates are dropped; an existing session (same date, same account) absorbs new trades — you can stack exports over weeks. The Executions export has no MAE/MFE.',
            'El PnL se recalcula desde los precios y el valor del punto del registro CME, y luego se restan las comisiones; la columna Profit sirve de control. Un instrumento ausente del registro se ignora y se señala (instrumento no reconocido). Los duplicados exactos se descartan; una sesión existente (misma fecha, misma cuenta) absorbe los nuevos trades — puede apilar exportaciones a lo largo de las semanas. La exportación Executions no contiene MAE/MFE.',
          )}
        </p>
        {report && (
